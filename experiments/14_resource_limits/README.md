...
查询进程的文件描述符限制，安全地临时降低软限制后恢复，并读取进程资源用量。
...
程序比较 `getrlimit` 与 `prlimit` 的查询结果，把 `RLIMIT_NOFILE` 软限制临时降到不高于 64，确认修改生效后立即恢复原值。它不尝试打开大量文件，因此不会真正耗尽资源。最后校验并输出 `getrusage` 的时间字段。
